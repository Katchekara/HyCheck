...
export default function VerifierPage({ params }) {
  ...
  async function handleVerify() {
    ...
    if (selected === "texte") {
      endpoint = "http://127.0.0.1:8000/api/text/analyze";
      options = {
        method: "POST",
      ...
      endpoint = "http://127.0.0.1:8000/api/audio/transcribe";
      const formData = new FormData();
      formData.append("file", file);
      options = { method: "POST", body: formData };
    } else if (selected === "image") {
      endpoint = "http://127.0.0.1:8000/api/image/analyze-image";
      const formData = new FormData();
      formData.append("file", file);
      options = { method: "POST", body: formData };
    }
  ...
  return (
    <div className="space-y-6 pt-2">
      <div className="flex items-center justify-between text-sm">
        ...
        <span className="font-mono text-xs text-muted">{result?.id || params.id}</span>
      </div>
